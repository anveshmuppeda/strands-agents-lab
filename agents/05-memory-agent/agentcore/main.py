"""
Gateway Agent with Memory — Deployed on AgentCore Runtime (Part 5)

Same Gateway agent from Guide 04, but with AgentCore Memory added.
The agent remembers conversations across sessions using AgentCoreMemorySessionManager.
...
# Global agent + session manager — reinitialize when session changes
_agent = None
...
def get_or_create_agent(actor_id: str, session_id: str) -> Agent:
    """Create agent with Gateway tools and Memory session manager."""
    global _agent, _session_manager, _current_session_id

    # Reinitialize if session changed
    ...
    # Memory session manager — handles save/load automatically
    ...
    _agent = Agent(
        ...
        tools=tools_list,
        session_manager=_session_manager,  # ← This is the only addition for memory
    )
